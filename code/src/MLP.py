...
result_dir = parent_parent_dir+f'/result/MLP_merge_{ESM_name}_{task_name}/'
...
class PepDataset(Dataset):
    def __init__(self, csv_file, ids_file):
        ...
        feature_embeddings = np.loadtxt(feature_path)
    ...
    #     labels = data[TASTE_COLUMNS].values
    #     feature_embeddings = np.loadtxt(feature_path)
    #     self.features = feature_embeddings
    #     self.labels = labels
    #     self.feature_dim = self.features.shape[1]
...
class PepUpDataset(Dataset):
    def __init__(self, csv_file, ids_file):
        data = pd.read_csv(csv_file, sep='\t')
        labels = data[TASTE_COLUMNS].values

        feature_embeddings1 = np.loadtxt(f"/home/hy/Documents/protein-peptide/umamimeta/dataset/umami_pep_dataset/{task_name}/esm2_t6_8M_UR50D.txt")
        # feature_embeddings2 = np.loadtxt("/home/hy/Documents/protein-peptide/umamimeta/dock/all_feature.txt")
        feature_embeddings2 = np.loadtxt(f"/home/hy/Documents/protein-peptide/umamimeta/dataset/umami_pep_dataset/{task_name}/{task_name}_feature.csv")
        merged_embeddings = np.hstack((feature_embeddings1, feature_embeddings2))
        self.selected_ids = load_ids(ids_file)
        self.features = merged_embeddings[self.selected_ids]
...
for fold in range(1,6):
    train_dataset = PepUpDataset(task_dir+f'/{task_name}_processed.csv', task_dir+f'/fold{fold}_train_ids.txt')
    input_size = train_dataset.feature_dim
    test_dataset = PepUpDataset(task_dir+f'/{task_name}_processed.csv', task_dir+f'/fold{fold}_test_ids.txt')
