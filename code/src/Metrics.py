...
if __name__ == "__main__":
    ESM_name = "esm2_t6_8M_UR50D" #esm2_t6_8M_UR50D esm1v_t33_650M_UR90S_1
    task_name = "Kd"
    result_dir = f'result/{task_name}_{ESM_name}/'
    # for fold in range(1,6):
    #     if fold == 1:
    #         preds = np.loadtxt(result_dir+f'/fold{fold}_val_preds.txt')
    #         labels = np.loadtxt(result_dir+f'/fold{fold}_val_labels.txt', dtype=int)
    #     else:
    ...
    # bi_classify_metrics(labels, preds, plot_cm=True, plot_auc=True, save_path_name=result_dir+'/bi',classnames=['Negative','Positive'])
    ...
    regression_metrics(labels, preds, plot_regress=True, save_path_name=result_dir+'/reg')